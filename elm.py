...
class MetricELM:
    def __init__(self, hidden_num, c, seed, activation='sigmoid'):
    ...
    def fit(self, x, y):
        ...
        # print(self.hidden_num, 'hidden nodes')
        feature_size = x.shape[1]
        rng = np.random.RandomState(self.seed)
        self.w = rng.random([feature_size, self.hidden_num]) * 2 - 1
        self.b = rng.random([self.hidden_num]) * 2 - 1
    ...
    def _build_x_pairs(x):
        ...
        # mask = np.tril(np.ones(n)).astype(bool)
        x = x[mask]
        return x
